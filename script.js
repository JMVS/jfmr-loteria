...
function agregarNumero() {
  ...
  if (numero >= 1 && numero <= 90 && !numerosIngresados.includes(numero)) {
	...
	mostrarMensajeError('Número no válido: Fuera de rango o repetido.');
  }
  prepararInputNumero();
}
...
// Atajos de teclado
document.addEventListener('keydown', (e) => {
    const key = e.key.toLowerCase(); // Caso no sensitivo
    if (e.ctrlKey && e.altKey) {
		e.preventDefault();
        switch (key) {
            case 'b': // Ctrl+Alt+T: Mostrar ventana emergente para BINGO
                mostrarVentanaEmergente(textoBingo);
                break;
            case 'l': // Ctrl+Alt+T: Mostrar ventana emergente para LINEA
            ...
            case 'n': // Ctrl+Alt+N: Nueva partida
                ...
                break;
            default:
                // Acción predeterminada en caso de que ninguna tecla coincida
                break;
        }
    }
});
